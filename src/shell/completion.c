...
static void ml_push(MatchList *m, char *s) {
    ...
    if (m->count >= m->cap) {
        ...
        size_t bytes = (size_t)new_cap * sizeof(char *);
        char **new_items = NULL;
        if (m->items) {
            new_items = (char **)HeapReAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, m->items, bytes);
        } else {
            new_items = (char **)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, bytes);
        }
        if (!new_items) {
...
static void collect_files(const char *prefix, const char *dir, MatchList *m) {
    ...
    wchar_t *ws = u8_to_u16(search, NULL);
    if (!ws) return;
    ...
    do {
        ...
        char *name = u16_to_u8(fd.cFileName, NULL);
        if (!name) continue;
