# Improve Error Handling

## Purpose

Make failures clearer and safer without hiding errors.

## When to use

Use when errors are swallowed, vague, or unsafe.

## Inputs

Task; relevant files; constraints

## Read first

- [Context_Policy](../../CONTEXT_POLICY.md)
- [Project Summary](../../memory/project-summary.md)
- [Commands](../../memory/commands.md)

## Steps

1. Read minimal context.
2. inspect relevant files.
3. make a small plan.
4. apply focused work.
5. verify.
6. document result.

## Output

Improve Error Handling result with short explanation and links.

## Verification

Run the nearest relevant build, test, or manual check.

## Next skills

- [Regression Check](../testing/regression-check.md)
- [Write Dev Notes](../docs/write-dev-notes.md)

## Anti-patterns

- Do not make unrelated changes
- Do not invent project facts
- Do not skip verification.
