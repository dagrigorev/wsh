...
TEST(Screen, OutputKeepsScrolledViewportAnchored) {
    ScreenBuffer sb;
    screen_init(&sb, 4, 2, 8);
    ...
    screen_newline(&sb);
    screen_carriage_return(&sb);
    for (int i = 0; i < 4; ++i) screen_put_char(&sb, 'D', NULL);
...
TEST(Screen, ResizePreservesScrollback) {
    ScreenBuffer sb;
    screen_init(&sb, 4, 2, 8);
